...
export function PieChartDonut({ card, cash, online }: { card: number; cash: number; online: number }) {
  ...
  const percentageDifference = cash > 0 ? ((online - cash) / cash) * 100 : online > 0 ? 100 : 0;
    ...
    <Card className="flex flex-col">
      ...
      <CardContent className="flex-1 pb-0">
        <ChartContainer config={chartConfig} className="mx-auto aspect-square max-h-[250px]">
          <PieChart>
            ...
            <Pie data={chartData} dataKey="transactions" nameKey="mode" innerRadius={60} strokeWidth={5}>
              <Label
                content={({ viewBox }) => {
      ...
      <CardFooter className="flex-col gap-2 text-center text-sm">
        ...
        <div className="leading-none text-muted-foreground">
          Data reflects transactions for the year {new Date().getFullYear()}.
        </div>
      </CardFooter>
    </Card>
  );
